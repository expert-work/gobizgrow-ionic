...
export class NewPage implements OnInit {
  ...
  ngOnInit(){
    this.authService.userData$.subscribe((res:any) => {
     ...
     if(  this.data  !== undefined){ 
      ...
      }else{ this.router.navigate(['app/jobs']) }
   }
  ...
  saveUpdateForm(form:any){
              this.apisService.jobUpdate(form).subscribe((result: any) => {
                if(result.data){
                    ...
                    this.router.navigate(['app/jobs'], navigationExtras);
                } 
              },
              (error: any)=>{
                      if(error.status==0){
   ...
   saveAddForm(form:any){
                  this.apisService.jobAdd(form).subscribe((result: any) => {
                    if(result.data){
                        let navigationExtras: NavigationExtras = {
                          state: {
                              data:  'refresh'
